...
public class FrontGoodsController {
	...
	public String detailNews(Integer goodsid, Model model, HttpSession session) {
	...
	public String searchGoods(@RequestParam(value = "page", defaultValue = "1") Integer pn, String keyword, Model model,
		...
		goodsExample.or().andGoodsnameLike("%" + keyword + "%")
				.andCategoryNotIn(Arrays.asList(new Integer[] { 1, 2, 3 }));
		List<Goods> goodsList = goodsService.selectByExample(goodsExample);

		// 获取图片地址
		for (int i = 0; i < goodsList.size(); i++) {
			Goods goods = goodsList.get(i);
			if (goods.getGoodsname().length() > 19) {
				goods.setGoodsname(goods.getGoodsname().substring(0, 18) + "...");
			}
	...
	public String getCateGoods(String cate, @RequestParam(value = "page", defaultValue = "1") Integer pn, Model model,
			HttpSession session) {
		User user = (User) session.getAttribute("user");

		// 查询分类id
		CategoryExample categoryExample = new CategoryExample();
		categoryExample.or().andCatenameLike(cate).andCateidNotIn(Arrays.asList(new Integer[] { 1, 2, 3 }));
		List<Category> categoryList = cateService.selectByExample(categoryExample);

		//
		CategoryExample categoryExampleAll = new CategoryExample();
		categoryExampleAll.or().andCateidNotIn(Arrays.asList(new Integer[] { 1, 2, 3 }));
		List<Category> categoryListAll = cateService.selectByExample(categoryExampleAll);
		...
		if (!cateId.isEmpty()) {
			...
			goodsExample.or().andCategoryIn(cateId).andCategoryNotIn(Arrays.asList(new Integer[] { 1, 2, 3 }));
		}
		List<Goods> goodsList = goodsService.selectByExample(goodsExample);
		...
		for (int i = 0; i < goodsList.size(); i++) {
			...
			if (goods.getGoodsname().length() > 18) {
	...
	@RequestMapping("/news")
	public String getCateNews(String cate, @RequestParam(value = "page", defaultValue = "1") Integer pn, Model model,
			HttpSession session) {
		User user = (User) session.getAttribute("user");

		// 查询分类id
		CategoryExample categoryExample = new CategoryExample();
		categoryExample.or().andCatenameLike(cate).andCateidIn(Arrays.asList(new Integer[] { 1, 2, 3 }));
		List<Category> categoryList = cateService.selectByExample(categoryExample);

		//
		CategoryExample categoryExampleAll = new CategoryExample();
		categoryExampleAll.or().andCateidIn(Arrays.asList(new Integer[] { 1, 2, 3 }));
		List<Category> categoryListAll = cateService.selectByExample(categoryExampleAll);
		...
		// 查询数据
		GoodsExample goodsExample = new GoodsExample();
		goodsExample.or().andDetailcateLike("%" + cate + "%").andCategoryIn(Arrays.asList(new Integer[] { 1, 2, 3 }));
		if (!cateId.isEmpty()) {

			goodsExample.or().andCategoryIn(cateId).andCategoryIn(Arrays.asList(new Integer[] { 1, 2, 3 }));
		}
		List<Goods> goodsList = goodsService.selectByExample(goodsExample);
